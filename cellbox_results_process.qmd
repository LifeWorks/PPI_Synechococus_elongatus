...
```{python}
results_dir = home +"/workspace/data/ppi3/cellbox/results/"
```

```{python}
analyses_dir = home + "/Dropbox/PNNL/PredPheno/TA3_multiscale_model/S.elongatus/cellbox/RNA/20230608_analyses/"
experiment = "20230405_cyano_rna"
...
```{python}
new_results_dir = results_dir + experiment + "_top1k/"
os.makedirs(new_results_dir) if not os.path.exists(new_results_dir) else None
scores_1000 = scores_1000.reset_index(drop=True)
scores_1000.to_csv(new_results_dir + "scores_top1k.csv")
scores = scores_1000.copy()
for i in range(scores.shape[0]):
...
Now we can process all the five new experiemnts

```{python}
for experiment in ["Cellbox_t" + str(i) for i in range(1,6)]:
...
```{python}
results_dir = home +"/workspace/data/ppi3/cellbox/results/20230807/"
for experiment in ["central_carbon" + str(i) for i in ["", "_de", "_ps", "_ps_de"]]:
    ...
    for i in range(scores.shape[0]):
        seed = '{:0>{}}'.format(str(scores.iloc[i, 0]), 3)
        rep = str(scores.iloc[i, 1])
        new_results_subdir = new_results_dir + "rank_" + str(i) + "_seed_" + seed + "_rep_" + rep + "/"
        os.makedirs(new_results_subdir) if not os.path.exists(new_results_subdir) else None
        files = [listed for listed in os.listdir(result_dir + "seed_" + seed) if (rep + "_best" in listed) or (experiment in listed)]
        for filename in files:
            shutil.copy2(result_dir + "seed_" + seed + '/' + filename, new_results_subdir + filename)

        shutil.copy2(result_dir + "seed_" + seed + '/' + "record_eval.csv", new_results_subdir + "record_eval.csv")
        shutil.copy2(result_dir + "seed_" + seed + '/' + "random_pos.csv", new_results_subdir + "random_pos.csv")
```
