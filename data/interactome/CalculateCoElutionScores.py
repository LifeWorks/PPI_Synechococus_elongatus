...
class ElutionData():
	...
	def orthmap(self, mapping):
		...
		newprot2Index = {}
		i = 0
		for prot, index in list(self.prot2Index.items()):
	...
	def printMat(self, mat):
		out = "ProtiID\tFraction_" + "\tFracion_".join(map(str, list(range(1,240))))
...
def calculateScore_MI(a, b):
	...
	joint_probs = np.array([len(x)/numFracs for x in [a_upper&b_upper, a_upper&b_lower, a_lower&b_upper, a_lower&b_lower]])
...
def entropy(probs, base=0):
	...
	return -sum(probs * [math.log(x, base) for x in tmp_probs])
...
class Genemania:
	...
	def catchFile(self): 
		...
		r = urllib.request.urlopen(speciesURL).read()
...
#		data holders
		...
		for i, f_evidence in enumerate(self.functionalEvidences):
			...
			self.scoreCalc.scores = np.zeros((len(list(scores.keys())), len(self.functionalEvidences)))
			i = 0
			for edge in scores:
	...
	def map_proteinNames(self):
		...
		for key, value in unipront_geneNames_dic.items():
...
class CalculateCoElutionScores():
	...
	def get_val_proteins(self):
		...
		for items in list(self.ppiToIndex.keys()):
	...
	def calculateScores(self, toPred, tokeep=set([])):
		...
		print("\t".join(self.header), file=outFH)
		k = 0
		ppi_index = 0
		write_buffer = ""
		for ppi in toPred:
	...
	def getAllPairs(self):
		...
		for elution_data in self.elutionData:
			...
			allprots = list(elution_data.prot2Index.keys())
			candidatePPis = set([])
	...
	def readTable(self, scoreF, gs=""):
		...
		for line in scoreFH:
			...
			edge_scores = np.nan_to_num(np.array(list(map(float, linesplit[2:]))))
...
class STRING:
	...
	def load_string(self):
		...
		self.scoreCalc.scores = np.zeros((len(list(temp_score_dict.keys())), len(usefulScores)))
